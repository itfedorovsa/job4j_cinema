...
public class PostgresUserRepository implements UserRepository {
    ...
    public void update(User user) {
        try (Connection cn = pool.getConnection(); PreparedStatement ps = cn.prepareStatement(UPDATE)) {
            ps.setString(1, user.getName());
            ps.setString(2, user.getEmail());
            ps.setString(3, user.getPhone());
    ...
    public Optional<User> findByEmail(String email) {
        Optional<User> rsl = Optional.empty();
        try (Connection cn = pool.getConnection(); PreparedStatement ps = cn.prepareStatement(SELECT_EMAIL)) {
    ...
    public Optional<User> findById(int id) {
        Optional<User> rsl = Optional.empty();
        try (Connection cn = pool.getConnection(); PreparedStatement ps = cn.prepareStatement(SELECT_ID)) {
            ps.setInt(1, id);
            try (ResultSet it = ps.executeQuery()) {
                if (it.next()) {
                    rsl = Optional.of(newUser(it));
    ...
    public Optional<User> findByPhone(String phone) {
        Optional<User> rsl = Optional.empty();
        try (Connection cn = pool.getConnection(); PreparedStatement ps = cn.prepareStatement(SELECT_PHONE)) {
    ...
    public Optional<User> findByEmailAndPhone(String email, String phone) {
        Optional<User> rsl = Optional.empty();
        try (Connection cn = pool.getConnection(); PreparedStatement ps = cn.prepareStatement(SELECT_EMAIL_AND_PHONE)) {
            ps.setString(1, email);
    ...
    /**
     * Standalone method for creating {@link ru.job4j.cinema.model.User} object
     *
    ...
    private User newUser(ResultSet rslSet) throws SQLException {
        return new User(rslSet.getInt("u_id"), rslSet.getString("u_name"), rslSet.getString("u_email"), rslSet.getString("u_phone"));
    }
